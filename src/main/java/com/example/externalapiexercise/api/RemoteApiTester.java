...
public class RemoteApiTester implements CommandLineRunner {
    ...
    public Mono<Gender> getGender(String name){
        WebClient client = WebClient.create();
        Mono<Gender> gender = client.get().uri("https://api.genderize.io?name="+name)
                .retrieve()
                .bodyToMono(Gender.class);
        return gender;
    }
    public Mono<Age> getAge(String name){
        WebClient client = WebClient.create();
        Mono<Age> age = client.get().uri("https://api.agify.io?name="+name)
                .retrieve()
    ...
    public Mono<Nationality> getNationality(String name){
        WebClient client = WebClient.create();
        Mono<Nationality> nationality = client.get().uri("https://api.nationalize.io?name="+name)
                .retrieve()
...
/*        System.out.println(getGenderForName("Frederik").block());

        callSlowEndpointNonBlocking();
        getGendersBlocking();
        getGendersNonBlocking();
        CombinedResponse res = fetchNameDetails("Frederik").block();
        System.out.println(res);

        System.out.println(getNationality("Frederik").block());*/
    }
